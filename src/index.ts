export default findv;

/**
 * A findv function that takes any object with a nested structure and a path,
 ...
 *
 */
function findv(source: any, path: string, shouldThrow: boolean = false) {
    ...
    return parts.reduce((acc, el) => {
        if (acc === undefined) {
            if (shouldThrow) {
                throw new Error(`Could not findv the value using path: ${path}`);
            } else {
                return undefined;
            }
        }
